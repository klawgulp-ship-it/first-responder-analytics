...
router.get('/', async (req, res) => {
    try {
        const [activeIncidents, unitStatus, todayIncidents, incidentsByType, avgResponse, recentIncidents, districtPerf,] = await Promise.all([
            // Active incidents
            (0, connection_1.query)(`SELECT COUNT(*) as count FROM incidents WHERE status NOT IN ('closed', 'cancelled', 'resolved')`),
            // Units by status
            (0, connection_1.query)(`SELECT status, COUNT(*) as count FROM units GROUP BY status`),
            // Today's incidents
            (0, connection_1.query)(`SELECT COUNT(*) as count FROM incidents WHERE created_at >= CURRENT_DATE`),
            // Incidents by type today
            (0, connection_1.query)(`SELECT incident_type, COUNT(*) as count FROM incidents WHERE created_at >= CURRENT_DATE GROUP BY incident_type`),
            // Average response time (last 7 days)
            (0, connection_1.query)(`SELECT AVG(response_time_seconds) as avg_response FROM incidents WHERE created_at >= CURRENT_DATE - INTERVAL '7 days' AND response_time_seconds IS NOT NULL`),
            // Recent incidents
      ...
      `),
        ...
        for (const row of unitStatus.rows) {
            ...
            if (row.status === 'available')
                availableUnits = parseInt(row.count);
        }
        const typeBreakdown = {};
