...
router.get('/', async (req, res) => {
    try {
        const { page, limit, offset } = (0, validation_1.paginate)(req);
        const { type, status, priority, district_id, from, to, search } = req.query;
        let where = 'WHERE 1=1';
        const params = [];
        let paramIdx = 1;
        if (type) {
            where += ` AND i.incident_type = $${paramIdx++}`;
            params.push(type);
        }
        if (status) {
        ...
        if (priority) {
            where += ` AND i.priority = $${paramIdx++}`;
            params.push(priority);
        }
        if (district_id) {
            where += ` AND i.district_id = $${paramIdx++}`;
            params.push(district_id);
        }
        if (from) {
            where += ` AND i.created_at >= $${paramIdx++}`;
            params.push(from);
        }
        if (to) {
            where += ` AND i.created_at <= $${paramIdx++}`;
        ...
        const countResult = await (0, connection_1.query)(`SELECT COUNT(*) FROM incidents i ${where}`, params);
...
router.get('/:id', async (req, res) => {
    try {
        ...
        const [incident, units, outcome, dispatchLog] = await Promise.all([
            (0, connection_1.query)(`SELECT i.*, d.name as district_name FROM incidents i LEFT JOIN districts d ON d.id = i.district_id WHERE i.id = $1`, [id]),
      ...
      `, [id]),
            (0, connection_1.query)(`SELECT io.*, p.first_name || ' ' || p.last_name as reporting_officer FROM incident_outcomes io LEFT JOIN personnel p ON p.id = io.reporting_officer_id WHERE io.incident_id = $1`, [id]),
            (0, connection_1.query)(`SELECT dl.*, u.unit_number, p.first_name || ' ' || p.last_name as dispatcher_name FROM dispatch_log dl LEFT JOIN units u ON u.id = dl.unit_id LEFT JOIN personnel p ON p.id = dl.dispatcher_id WHERE dl.incident_id = $1 ORDER BY dl.timestamp`, [id]),
        ]);
...
const createIncidentSchema = zod_1.z.object({
    incident_type: zod_1.z.enum(['fire', 'ems', 'police', 'multi_agency', 'hazmat', 'rescue', 'traffic']),
    priority: zod_1.z.enum(['1', '2', '3', '4', '5']),
    address: zod_1.z.string().min(1),
    location_lat: zod_1.z.number().optional(),
    location_lng: zod_1.z.number().optional(),
    description: zod_1.z.string().min(1),
    caller_name: zod_1.z.string().optional(),
    caller_phone: zod_1.z.string().optional(),
    dispatch_notes: zod_1.z.string().optional(),
    district_id: zod_1.z.string().uuid().optional(),
});
router.post('/', (0, auth_1.authorize)('dispatcher', 'captain', 'chief', 'admin'), (0, validation_1.validate)(createIncidentSchema), async (req, res) => {
    try {
        const data = req.body;
...
const updateStatusSchema = zod_1.z.object({
    status: zod_1.z.enum(['dispatched', 'en_route', 'on_scene', 'resolved', 'closed', 'cancelled']),
});
router.patch('/:id/status', (0, auth_1.authorize)('dispatcher', 'captain', 'chief', 'admin'), (0, validation_1.validate)(updateStatusSchema), async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const result = await (0, connection_1.query)(`UPDATE incidents SET status = $1::incident_status ${extraFields}, updated_at = NOW() WHERE id = $2 RETURNING *`, [status, id]);
