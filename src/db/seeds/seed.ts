...
const districtIds = Array.from({ length: 6 }, () => uuid());
const stationIds = Array.from({ length: 12 }, () => uuid());
const unitIds = Array.from({ length: 30 }, () => uuid());
const personnelIds = Array.from({ length: 60 }, () => uuid());
const shiftIds = Array.from({ length: 3 }, () => uuid());
...
async function seed() {
  ...
  try {
    ...
    for (const d of districts) {
      await client.query(
        `INSERT INTO districts (id, name, district_number, population, area_sq_miles, risk_level)
         VALUES ($1, $2, $3, $4, $5, $6::risk_level)`,
        [d.id, d.name, d.num, d.pop, d.area, d.risk]
      );
    }
    ...
    for (const s of stations) {
      await client.query(
        `INSERT INTO stations (id, station_number, name, address, location_lat, location_lng, district_id, is_headquarters, capacity_units)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
        [s.id, s.num, s.name, s.addr, s.lat, s.lng, districtIds[s.dist], s.hq, s.cap]
      );
    }
    ...
    const statuses: string[] = ['available', 'available', 'available', 'available', 'dispatched', 'on_scene', 'out_of_service'];
    for (const u of units) {
      const status = statuses[Math.floor(Math.random() * statuses.length)];
      await client.query(
        `INSERT INTO units (id, unit_number, unit_type, station_id, status, current_location_lat, current_location_lng, in_service_date)
         VALUES ($1, $2, $3::unit_type, $4, $5::unit_status, $6, $7, $8)`,
        [u.id, u.num, u.type, stationIds[u.station],
         status,
    ...
    for (const s of shifts) {
      await client.query(
        `INSERT INTO shifts (id, shift_name, shift_pattern, start_time, end_time)
         VALUES ($1, $2, $3::shift_pattern, $4, $5)`,
        [s.id, s.name, s.pattern, s.start, s.end]
      );
    }
    ...
    const ranks: { rank: string; certs: string[] }[] = [
    ...
    for (let i = 0; i < 60; i++) {
      ...
      await client.query(
        `INSERT INTO personnel (id, badge_number, first_name, last_name, rank, unit_id, email, hire_date, certifications, status)
    ...
    for (const h of hazards) {
      await client.query(
        `INSERT INTO hazard_locations (name, hazard_type, address, location_lat, location_lng, district_id, risk_level, special_instructions)
         VALUES ($1, $2::hazard_type, $3, $4, $5, $6, $7::risk_level, $8)`,
        [h.name, h.type, h.addr, h.lat, h.lng, districtIds[h.dist], h.risk, h.instr]
      );
    }
    ...
    for (let d = 0; d < 6; d++) {
      ...
      for (const z of zones) {
        await client.query(
          `INSERT INTO response_zones (district_id, zone_name, zone_type, target_response_time_seconds, avg_response_time_seconds)
           VALUES ($1, $2, $3, $4, $5)`,
          [districtIds[d], z.name, z.type, z.target, z.target + Math.floor(Math.random() * 60) - 20]
        );
      }
    }
    ...
    // ============================================================
    const incidentTypes: { type: string; weight: number }[] = [
      { type: 'ems', weight: 45 },
      { type: 'fire', weight: 15 },
    ...
    const now = new Date();
    const incidentRecords: { id: string; type: string; createdAt: Date; distIdx: number }[] = [];
    ...
    for (let daysAgo = 90; daysAgo >= 0; daysAgo--) {
      ...
      for (let j = 0; j < dailyCount; j++) {
        ...
        await client.query(
          `INSERT INTO incidents (id, incident_number, incident_type, priority, status, location_lat, location_lng, address, district_id, description, response_time_seconds, scene_time_seconds, total_time_seconds, created_at, dispatched_at, resolved_at, closed_at, weather_conditions)
        ...
        for (let u = 0; u < Math.min(numUnits, 30); u++) {
          ...
          while (usedUnits.has(unitIdx)) unitIdx = (unitIdx + 1) % 30;
          ...
          await client.query(
            `INSERT INTO incident_units (incident_id, unit_id, is_primary, dispatched_at, en_route_at, on_scene_at, cleared_at, response_time_seconds)
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
            [incId, unitIds[unitIdx], u === 0, dispAt, enRouteAt, onSceneAt, clearedAt, responseTime + u * 30]
          );

          // Dispatch log entry
          await client.query(
            `INSERT INTO dispatch_log (incident_id, unit_id, action, dispatcher_id, notes, timestamp)
             VALUES ($1, $2, 'dispatched'::dispatch_action, $3, $4, $5)`,
            [incId, unitIds[unitIdx], personnelIds[Math.floor(Math.random() * 60)],
        ...
        if (isClosed) {
          ...
          await client.query(
            `INSERT INTO incident_outcomes (incident_id, outcome_type, injuries_civilian, injuries_responder, fatalities, property_damage_estimate, narrative, follow_up_required, reporting_officer_id)
    ...
    for (let daysAgo = 90; daysAgo >= 1; daysAgo--) {
      ...
      for (let d = 0; d < 6; d++) {
        ...
        if (distIncidents.length === 0) continue;

        const counts = { fire: 0, ems: 0, police: 0, multi_agency: 0, other: 0 };
        for (const inc of distIncidents) {
          if (inc.type in counts) (counts as Record<string, number>)[inc.type]++;
          else counts.other++;
        }

        await client.query(
          `INSERT INTO daily_stats (stat_date, district_id, fire_count, ems_count, police_count, multi_agency_count, other_count, total_incidents, avg_response_time_seconds, units_utilized_pct, personnel_on_duty)
